...
<body>
    ...
    <div class="news-container">
        <div class="row">
            <h1 class="home-title">Add Hint</h1>
            ...
            <div class="span12">
                <form:form class="form-horizontal" onsubmit="javascript:return false;">
                    <fieldset>
                            ...
                            <div class="col-lg-10">
                                ...
                                <select id="select01" class="form-control" style="width: 300px">
                            		...
                            		<c:forEach items="${allchalls}" var="ch">
                            			<option value="${ch.id}" <c:if test="${ param.taskid==ch.id }">selected</c:if>>${ch.title}</option>
                            		</c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary" onclick="doaddhint();">Save</button>
                    </fieldset>
                </form:form>
            </div>
        </div>
    ...
    <script type="text/javascript">
    	function doaddhint(){
    		...
    		$.post('admin/newhints.json',postdata ,function(data){
        		if (data.err==0) {
        			$.alert({
                	    title: false,
                	    content: data.errmsg,
                	    ...
                	    confirmButtonClass: 'btn-danger',
               	    	confirm: function(){
               	    		if (data.err==-99 || data.err==-98) {
               	    			...
               	    			window.location.replace('admin/addhint<%=(queryString!=null?("?"+queryString):"")%>');
               	    		}
